...
Sherpas.Util = (function () {
  ...
  function monthMeta(y,m){
    ...
    // CORRECCIÓN CRÍTICA: Cálculo exacto de semanas necesarias
    var totalCells = firstW + lastN; // Celdas desde inicio de semana hasta último día
    var weeks = Math.ceil(totalCells / 7); // Semanas completas necesarias
    
    console.log(`monthMeta(${y}/${m}): firstW=${firstW}, lastN=${lastN}, totalCells=${totalCells}, weeks=${weeks}`);
  ...
  function buildGuideMonthArrays(y,m,startRow){
    ...
    console.log(`buildGuideMonthArrays(${y}/${m}): weeks=${meta.weeks}, rowsNeeded=${rowsNeeded}, mtA1.length=${mtA1.length}`);
